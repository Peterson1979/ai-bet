    ...
    <div className="rounded-[24px] border border-cyan-400/20 bg-gradient-to-b from-[#0B1220] to-[#0F172A] p-6">
      ...
      {valid && kelly === 0 && (
        <div className="mt-5 rounded-xl border border-red-400/30 bg-red-500/10 p-4 text-center">
          <p className="text-sm text-red-300 font-semibold">No value bet — Kelly suggests no stake.</p>
        </div>
      )}
    </div>
  );
...
function ValueBetFinder() {
  ...
  const impliedFair = valid ? (100 / fair) : null;
  const impliedBook = valid ? (100 / book) : null;
  const edge        = impliedFair && impliedBook ? (impliedFair - impliedBook) : null;
  ...
  return (
    <div className="rounded-[24px] border border-cyan-400/20 bg-gradient-to-b from-[#0B1220] to-[#0F172A] p-6">
      ...
      {valid && edge !== null && (
        <div className="mt-5">
          <div className={`rounded-xl border p-4 text-center ${isValue ? "border-emerald-400/30 bg-emerald-500/10" : "border-red-400/30 bg-red-500/10"}`}>
            ...
            <p className="mt-2 text-sm text-slate-300">
              {isValue
                ? "The bookmaker is offering better odds than your fair estimate. This is a value bet."
                : "The bookmaker odds are lower than your fair estimate. Avoid this bet."
              }
            </p>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1500px] px-4 md:px-6 pb-16">
          ...
          <div className="py-12">
            ...
            <p className="mt-4 max-w-2xl text-slate-300">
              Free calculators and tools to help you bet smarter — odds converter, bet calculator, bankroll manager and value bet finder.
            </p>
          </div>
          ...
          <div className="mt-12 rounded-[24px] border border-cyan-400/20 bg-gradient-to-r from-[#0B1220] via-[#0F172A] to-[#111827] p-8 text-center">
            ...
            <a
              href="/betting"
              className="mt-6 inline-block rounded-full border border-cyan-400/30 bg-cyan-500/10 px-8 py-3 text-sm font-bold uppercase tracking-wider text-cyan-300 transition-all hover:bg-cyan-400/20 hover:border-cyan-300/60"
            >
